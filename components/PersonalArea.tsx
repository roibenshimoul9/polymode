...
const PersonalArea: React.FC<PersonalAreaProps> = ({ user, onLogout }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-20 text-right animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row-reverse items-center gap-10 mb-20 bg-[#111] p-10 rounded-[3rem] border border-white/5 shadow-2xl">
        ...
        <div className="flex-grow space-y-2 text-center md:text-right">
          ...
          <div className="flex flex-row-reverse items-center justify-center md:justify-start gap-4 mt-4">
            <span className="bg-white/5 px-4 py-1.5 rounded-full text-xs text-gray-400 border border-white/5">
              חבר ב-Polymode מאז: <span className="text-white font-black">{user.joinedAt}</span>
            </span>
            <span className="bg-blue-600/10 px-4 py-1.5 rounded-full text-xs text-blue-400 border border-blue-500/20 font-black">
              לקוח פרימיום
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-[#111] rounded-[2.5rem] border border-white/5 overflow-hidden">
            <div className="p-8 border-b border-white/5 flex justify-between items-center flex-row-reverse">
              <h2 className="text-2xl font-black text-white">הזמנות אחרונות</h2>
              <button className="text-xs text-blue-500 font-bold hover:underline">צפה בכל ההיסטוריה</button>
            </div>
            ...
            <div className="divide-y divide-white/5">
              {[
                ...
                <div key={order.id} className="p-6 hover:bg-white/[0.02] transition-colors flex items-center justify-between flex-row-reverse">
                  <div className="flex items-center gap-4 flex-row-reverse">
                    <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center">
                      <svg className="w-6 h-6 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" strokeWidth="2"/></svg>
                    </div>
                    <div>
                      <h4 className="font-bold text-white text-sm">{order.item}</h4>
                      <p className="text-[10px] text-gray-600">מספר הזמנה: {order.id} • {order.date}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-8">
                    ...
                    <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${
                      order.status === 'הושלם' ? 'bg-green-500/10 text-green-500' : 'bg-cyan-500/10 text-cyan-500'
                    }`}>
                      {order.status}
                    </span>
                    <button className="p-2 bg-white/5 hover:bg-white/10 rounded-xl transition-all">
                      <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" strokeWidth="2"/></svg>
                    </button>
                  </div>
                </div>
              ))}
          ...
          <section className="bg-[#111] rounded-[2.5rem] border border-white/5 p-8">
            <h2 className="text-2xl font-black text-white mb-8">ספריית הקבצים שלי</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                { name: 'Ronin_Cyberpunk_V1.stl', size: '42 MB', format: 'STL' },
                { name: 'Mezuzah_Shell_A.step', size: '12 MB', format: 'STEP' }
              ].map((file, i) => (
                <div key={i} className="p-6 bg-black/40 rounded-3xl border border-white/5 flex items-center justify-between flex-row-reverse group hover:border-blue-500/30 transition-all">
                  <div className="flex items-center gap-3 flex-row-reverse">
                    <div className="w-10 h-10 bg-blue-600/10 text-blue-500 rounded-xl flex items-center justify-center font-black text-[10px] group-hover:bg-blue-600 group-hover:text-white transition-all">
                      {file.format}
                    </div>
                    <div>
                      <h5 className="text-xs font-bold text-gray-200">{file.name}</h5>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[#111] rounded-[2.5rem] border border-white/5 p-8 space-y-6">
            <h3 className="text-lg font-black text-white">הגדרות חשבון</h3>
            <ul className="space-y-1">
              {['פרטי התקשרות', 'כתובות למשלוח', 'שיטות תשלום', 'התראות במייל'].map(item => (
                <li key={item}>
                  <a href="#" className="block p-3 rounded-xl hover:bg-white/5 text-gray-400 hover:text-white text-sm transition-all flex items-center justify-between flex-row-reverse">
                    <span>{item}</span>
