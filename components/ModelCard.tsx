...
const ModelCard: React.FC<ModelCardProps> = ({ model, onAddToCart, onOpenDetails }) => {
  ...
  return (
    <div 
      className="group bg-[#111] border border-white/5 rounded-2xl overflow-hidden hover:border-blue-500/50 transition-all duration-300 flex flex-col h-full cursor-pointer"
      ...
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
          {model.images.map((_, idx) => (
            <div 
              key={idx} 
              className={`w-1.5 h-1.5 rounded-full transition-all ${idx === currentImgIndex ? 'bg-blue-500 w-3' : 'bg-white/30'}`}
            />
          ))}
        </div>
      ...
      <div className="p-4 flex-grow flex flex-col text-right">
        ...
        <div className="mt-auto pt-4 border-t border-white/5 space-y-3">
          <div className="flex items-baseline space-x-2 space-x-reverse">
            {model.price === 0 ? (
              <span className="text-lg font-bold text-cyan-400">בתיאום אישי</span>
            ) : (
              <>
          ...
          <div className="grid grid-cols-1 gap-2">
            {model.price === 0 ? (
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 bg-cyan-600 hover:bg-cyan-500 rounded-xl text-xs font-bold text-white transition-all shadow-lg shadow-cyan-900/20"
              >
                צרו קשר לייעוץ
              </button>
            ) : (
